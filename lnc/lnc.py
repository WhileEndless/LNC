...
default_values={
    ...
    "patterns": {
        ...
        "credit_card": [  
            r"4[\d\s]{12}(?:[\d\s]{3})?",
            r"(?:5[1-5]|2[2-7])[\d\s]{14}",
            r"6(?:011|5[\d\s]{2})[\d\s]{12}",
            r"3[47][\d\s]{13}",
            r"3(?:0[0-5]|[68][\d\s])[\d\s]{11}",
            r"(?:2131|1800|35[\d\s]{3})[\d\s]{11}",
...
def parse_args():
    ...
    def add_connection_args(parser, parallel:bool=True):
        ...
        connection_group.add_argument('--retry', type=int, dest='retry_count', 
                                     help=f'Number of retry attempts (default: {default_values["retry_count"]})')
        connection_group.add_argument('--delay', type=float, dest='delay_before_retry', 
                                     help=f'Delay in seconds before retrying (default: {default_values["delay_before_retry"]})')
        connection_group.add_argument('--timeout', type=float, 
                                     help=f'Connection timeout in seconds (default: {default_values["timeout"]})')
        if parallel:
            connection_group.add_argument('--max-jobs', type=int, dest='max_parallel_job', 
                                         help=f'Maximum number of parallel jobs (default: {default_values["max_parallel_job"]})')
        return connection_group
    def add_thread_args(parser):
        operation_group = parser.add_argument_group('Operation settings')
        operation_group.add_argument('-w', '--threads', type=int, dest='thread', 
                                    help=f'Number of worker threads (default: {default_values["thread"]})')
        return operation_group
    def add_settings_args(parser):
        setting_group = parser.add_argument_group('Settings')
        setting_group.add_argument('-c', '--config', type=str, help='Path to YAML configuration file')
        setting_group.add_argument('-o', '--output', type=str, 
                                  help=f'Output file prefix (default: {default_values["output"]})')
        setting_group.add_argument('--no-text-output', action='store_true', dest='disable_output_text', 
                                  help='Disable text output files')
        setting_group.add_argument('--enable-error-output', action='store_true', 
                                  help='Enable error messages in console output')
        setting_group.add_argument('--enable-end-prefix', action='store_false', dest='disable_output_end_prefix', 
                                  help='Disable unique suffix in output filenames (allows overwriting)')
        setting_group.add_argument('--log-errors', action='store_true', dest='write_errors_to_file', 
                                  help='Write errors to a separate log file')
        return setting_group
    def add_download_settings_args(parser):
        parser.add_argument('--download_folder', type=str, 
                           help=f'Directory to save downloaded files (default: {default_values["download_folder"]})')
    ...
    def add_smb_shares_filter_args(parser):
        parser.add_argument('--ignore_shares', dest='smb_files_ignore_shares', type=str, nargs='+', 
                           help='Skip these SMB shares. Example: --ignore_shares ipc$ print$ (default: ipc$ print$)')
    def add_download_always_args(parser):
        parser.add_argument('--always_download', type=str, nargs='+', 
                           help=r'Download files matching these regex patterns. Example: --always_download \.txt$ \.docx$')
        parser.add_argument('--max_download_size', type=int, 
                           help=f'Maximum file size in MB for download/analysis (default: {default_values["max_download_size"]}MB)')
    def add_smb_args(parser):
        parser.add_argument('--port', dest="smb_port", type=int, 
                           help=f'SMB port (default: {default_values["smb_port"]})')
    
    def add_ftp_args(parser):
        parser.add_argument('--port', dest="ftp_port", type=int, 
                           help=f'FTP port (default: {default_values["ftp_port"]})')
    ...
    if 'command' in merged_config:
        if merged_config['command'] == 'smb' and merged_config.get('smb_command') == 'analyze':
            if 'targets_file' in merged_config and 'files_file' not in merged_config:
                merged_config['files_file'] = merged_config['targets_file']
        elif merged_config['command'] == 'ftp' and merged_config.get('ftp_command') == 'analyze':
            if 'targets_file' in merged_config and 'files_file' not in merged_config:
                merged_config['files_file'] = merged_config['targets_file']
...
def check_existing_output_files(config):
    """Check for existing output files and ask user whether to delete them."""
    import glob
    from rich.prompt import Confirm
    
    output_prefix = config['output']
    if not config['disable_output_end_prefix']:
        # When output_end is used, files will have unique names, so no need to check
        return
    
    # Determine which files would be written by this command
    patterns = []
    
    # Base error file pattern (used by all commands)
    if config.get('write_errors_to_file'):
        patterns.append(f"{output_prefix}_errors.txt")
    
    # Command-specific patterns and consider process chain
    command = config.get('command')
    
    if command == 'smb':
        subcommand = config.get('smb_command')
        
        # Check if we need to go through the process chain
        using_target = 'target' in config or 'targets_file' in config
        
        # For crawl or analyze with target flag, we'll run the share command first
        if (subcommand == 'crawl' or subcommand == 'analyze') and using_target:
            patterns.extend([
                f"{output_prefix}_smb_share_list.json",
                f"{output_prefix}_smb_share_list.txt"
            ])
        
        # For analyze with target flag, we'll run the crawl command after shares
        if subcommand == 'analyze' and using_target and 'shares_file' not in config:
            patterns.extend([
                f"{output_prefix}_smb_files.json",
                f"{output_prefix}_smb_files.txt"
            ])
        
        # Now add the command-specific files
        if subcommand == 'share':
            patterns.extend([
                f"{output_prefix}_smb_share_list.json",
                f"{output_prefix}_smb_share_list.txt"
            ...
            # Analyze uses base output without module prefix for results
            patterns.extend([
                f"{output_prefix}.json",
                f"{output_prefix}.txt"
            ])
            # Also check for pattern-specific files
            for pattern_name in config.get('patterns', {}).keys():
                patterns.extend([
                    f"{output_prefix}_{pattern_name}.json",
                    f"{output_prefix}_{pattern_name}.txt"
                ])
    
    elif command == 'ftp':
        subcommand = config.get('ftp_command')
        
        # Check if we need to go through the process chain
        using_target = 'target' in config or 'targets_file' in config
        
        # For analyze with target flag, we'll run the crawl command
        if subcommand == 'analyze' and using_target and 'files_file' not in config:
            patterns.extend([
                f"{output_prefix}_ftp_files.json",
                f"{output_prefix}_ftp_files.txt"
            ])
        ...
        if subcommand == 'crawl':
            ...
            # Analyze uses base output without module prefix
            patterns.extend([
                f"{output_prefix}.json",
                f"{output_prefix}.txt"
            ])
    ...
    # Find existing files that match our patterns
    existing_files = []
    for pattern in patterns:
        matched_files = glob.glob(pattern)
        existing_files.extend(matched_files)
    
    # Remove duplicates and sort
    existing_files = sorted(set(existing_files))
    
    if existing_files:
        console.print("\n[yellow]Found existing output files that would be written by this command chain:[/yellow]")
        for file in existing_files:
            console.print(f"  - {file}")
        
        # Instead of asking, simply inform user we're appending to existing files
        console.print("[yellow]Appending to existing files...[/yellow]")
